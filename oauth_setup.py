...
refresh token into .env so the router can run unattended afterwards.

Usage:
    python oauth_setup.py

Requires GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET to already be set in .env,
from a Google Cloud OAuth client with Gmail send, Drive read, and Forms
responses read access enabled. Opens a browser window for you to grant
consent as the mailbox owner (julienne@myadventuregroup.com.au).

Not needed in production, which reuses the same refresh token already
minted for sales_proposals_automation.
"""
...
def main() -> None:
    ...
    print("Saved GOOGLE_REFRESH_TOKEN to .env — the router can now run unattended.")
